...
class _TreeDashboardPageState extends State<TreeDashboardPage> with SingleTickerProviderStateMixin {
  ...
  Widget _buildCalendarTab() {
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: _buildSimpleCalendarGrid(),
        ),
      ],
    );
  }
        ...
        Expanded(
          child: GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return FutureBuilder<List<TreeActivity>>(
                ...
                builder: (context, snapshot) {
                  ...
                  return InkWell(
                    onTap: () {
                      _showDayActivities(date);
                    },
                    child: Container(
                      margin: const EdgeInsets.all(2),
                      decoration: BoxDecoration(
                        color: isToday ? Colors.teal.withOpacity(0.2) : Colors.transparent,
                        border: Border.all(
                  ...
                  return Column(
                    children: activities.take(3).map((activity) {
                      return ListTile(
                        ...
                        subtitle: Text(DateFormat('MMM d, yyyy').format(activity.date)),
                        onTap: () {
  ...
  Future<List<TreeActivity>> _getActivitiesForDay(DateTime date) async {
    ...
    final careTips = await _firestore
        .collection('care_tip_completions')
        .where('userId', isEqualTo: userId)
        .get();
    
    for (var doc in careTips.docs) {
      final data = doc.data();
    ...
    // Treatment progress for this day
    final treatments = await _firestore
        .collection('treatment_progress')
        .where('userId', isEqualTo: userId)
        .get();
    
    for (var doc in treatments.docs) {
      final data = doc.data();
      
      // Treatment started
      final startedDate = DateTime.parse(data['startedDate']);
      if (startedDate.isAfter(startOfDay) && startedDate.isBefore(endOfDay)) {
        activities.add(TreeActivity(
          id: '${doc.id}_start',
          treeId: data['treeId'],
  ...
  Future<List<TreeActivity>> _getUpcomingActivities() async {
    ...
    for (var treeDoc in trees.docs) {
      ...
      if (treeData['isDiseased'] == true && treeData['diseaseId'] != null) {
        ...
        for (var doc in treatments.docs) {
          final data = doc.data();
          if (data.containsKey('completedDate') && data['completedDate'] != null) continue;
            ...
            if (stepDoc.exists) {
              ...
              if (targetDate.isAfter(now)) {
                activities.add(TreeActivity(
                  id: doc.id,
                  treeId: treeId,
                  title: '$treeName: Complete Treatment Step $stepNumber',
                  description: stepData['instruction'] ?? 'Complete treatment step',
                  date: targetDate,
      ...
      final wateringTips = await _firestore
          .collection('care_tip_completions')
          .where('treeId', isEqualTo: treeId)
          .where('userId', isEqualTo: userId)
      ...
      if (wateringTips.docs.isNotEmpty) {
        final lastWateringData = wateringTips.docs.first.data();
        final lastWateringDate = DateTime.parse(lastWateringData['completedDate']);
        
        // Predict next watering (7 days after last)
        final nextWateringDate = lastWateringDate.add(const Duration(days: 7));
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        ...
                        title: Text(activity.title),
                        subtitle: Text(DateFormat('h:mm a').format(activity.date)),
                        onTap: () {
...
class _TreeDashboardPageState extends State<TreeDashboardPage>
  ...
  // Simplified stats tab that avoids overflow issues
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        _buildStatItem('Total Trees', stats['totalTrees'].toString(), Icons.forest),
                        _buildStatItem('Healthy', stats['healthyTrees'].toString(), Icons.check_circle, color: Colors.green),
                        _buildStatItem('Diseased', stats['diseasedTrees'].toString(), Icons.healing, color: Colors.orange),
                      ],
                    ),
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceAround,
                      children: [
                        _buildStatItem('Care Tips', stats['careTipsCompleted'].toString(), Icons.eco, color: Colors.green),
                        _buildStatItem('Treatments', stats['treatmentsCompleted'].toString(), Icons.medical_services, color: Colors.blue),
                      ],
                    ),
                  ],
                ),
          ...
          builder: (context, snapshot) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _buildSimpleTreeCard(tree);
              },
            );
          },
        ),
...
/*   Widget _buildStatsTab() {
        ...
        StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                final tree = trees[index];
                return _buildTreeCardWithCompletedActivities(tree);
              },
            );
          },
        ),
  ...
  Widget _buildTreeCardWithCompletedActivities(TreeModel tree) {
    return Card(
  ...
  /* // Updated _buildStatsTab method with pie chart and progress bars
  Widget _buildStatsTab() {
    final userId = _auth.currentUser?.uid;
    if (userId == null) {
      return const Center(child: Text('Please sign in to view statistics'));
    ...
    return ListView(
      children: [
        // Summary Statistics Card with Pie Chart
        FutureBuilder<Map<String, dynamic>>(
          future: _statsService.getStatsSummary(),
          builder: (context, snapshot) {
            if (!snapshot.hasData) {
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              // Activity summary
                              Text(
                                'Total Activities: ${stats['careTipsCompleted'] + stats['treatmentsCompleted']}',
                                style: const TextStyle(
              ...
              children: [
                ...
                if (tree.isDiseased && tree.diseaseId != null) ...[
                  ...
                  _buildProgressBarSection(
                    ...
                    completedFilter: (doc) =>
                        (doc.data() as Map<String, dynamic>).containsKey('completedDate') == true &&
...
Future<int> _getTotalTreatmentSteps(String diseaseId) async {
  ...
  return snapshot.docs.length > 0 ? snapshot.docs.length : 1; // Avoid division by zero
}*/
