...
class NotificationService {
  ...
  Future<void> scheduleWateringReminder(TreeModel tree) async {
    ...
    final wateringRemindersEnabled = prefs.getBool('watering_reminders_enabled') ?? true;
    ...
    if (lastWateringDoc.docs.isNotEmpty) {
      ...
      nextWateringDate = lastWateringDate.add(Duration(days: wateringIntervalDays));
    } else {
  ...
  Future<void> scheduleTreatmentReminder(TreeModel tree) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    final treatmentRemindersEnabled = prefs.getBool('treatment_reminders_enabled') ?? true;
    ...
    for (var doc in treatmentQuery.docs) {
      ...
      final recommendedCompletionDate = startedDate.add(Duration(days: recommendedDays));
      
      // Schedule reminder for 80% of the way through the recommended period
      final reminderDate = startedDate.add(Duration(days: (recommendedDays * 0.8).round()));
      ...
      if (recommendedCompletionDate.isAfter(DateTime.now())) {
        await _scheduleLocalNotification(
          id: _generateNotificationId(tree.id, 'treatment_completion_${stepDoc.id}'),
          ...
          payload: 'treatment:${tree.id}:${tree.diseaseId}:${stepDoc.id}:complete',
        );
  ...
  Future<void> scheduleCareTipReminders(TreeModel tree) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    final careTipsRemindersEnabled = prefs.getBool('care_tips_reminders_enabled') ?? true;
    ...
    final completedTipIds = completedTipsQuery.docs.map((doc) => doc.data()['tipId'] as String).toSet();
  ...
  Future<void> scheduleFertilizationReminder(TreeModel tree) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    final fertilizationRemindersEnabled = prefs.getBool('fertilization_reminders_enabled') ?? true;
    ...
    final fertilizationDocs = await Future.wait(lastFertilizationQuery.docs.map((doc) async {
    ...
    filteredDocs.sort((a, b) => 
      DateTime.parse(b!.data()['completedDate']).compareTo(
        DateTime.parse(a!.data()['completedDate'])
      )
    );
    ...
    if (filteredDocs.isNotEmpty) {
      ...
      final lastFertilizationDate = DateTime.parse(filteredDocs.first!.data()['completedDate']);
      final fertInterval = _getFertilizationIntervalForTree(tree);
      nextFertilizationDate = lastFertilizationDate.add(Duration(days: fertInterval));
    } else {
  ...
  Future<void> cancelTreeReminders(String treeId) async {
    ...
    await _localNotifications.cancel(_generateNotificationId(treeId, 'watering'));
  ...
  Future<void> saveNotificationPreferences({
    ...
    await prefs.setBool('fertilization_reminders_enabled', fertilizationReminders);
